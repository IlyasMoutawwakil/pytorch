...
# prefer non-CPU devices over CPU devices. This is particularly useful
# in full compiled mode where intermediate tensors with device mismatches
# between CPU and another device (e.g., MTIA) represent only logical
# differences during compilation - these intermediate tensors will never
# physically materialize in the binary execution, so the device mismatch
# is not a real runtime concern. Enabling this allows the compiler to
# proceed with compilation by choosing the non-CPU device for consistency.
fake_tensor_prefer_non_cpu_device = False

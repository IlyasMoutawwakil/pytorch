...
class FakeTensorPreferNonCpu(TestCase):
    @unittest.skipIf(not RUN_CUDA, "requires cuda")
    def test_fake_tensor_prefer_non_cpu_device(self):
        """
        Test that fake_tensor_prefer_non_cpu_device configuration works correctly
        ...
        # Test with prefer_non_cpu_device enabled
        with torch._functorch.config.patch(fake_tensor_prefer_non_cpu_device=True):
            with FakeTensorMode():
                cuda_tensor = torch.randn(3, 4, device="cuda")
                ...
                # The result should be on CUDA device (non-CPU device is preferred)
    ...
    def test_fake_tensor_prefer_non_cpu_device_cpu_only(self):
        """
        Test that fake_tensor_prefer_non_cpu_device works correctly when only CPU tensors are involved.
        """
        with torch._functorch.config.patch(fake_tensor_prefer_non_cpu_device=True):
            with FakeTensorMode():
